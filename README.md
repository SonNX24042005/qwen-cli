...
*   **Tự động cập nhật phần mềm (Self-Update)**:
      ```
      /up
      ```
      *(Hoặc `/update`, tự động tải và thực thi kịch bản cập nhật mới nhất từ GitHub để nâng cấp phiên bản toàn cục).*
*   **Thoát ứng dụng**:
      ```
      /exit
      ```
...
Bạn có thể cấu hình trước các chế độ hoặc cập nhật khi khởi động:
*   Chuyển đổi chế độ: `node src/cli.js --mode fast` (hoặc `--think thinking`, `-t auto`)
*   Cập nhật ứng dụng: `qwen-cli update` (hoặc `qwen-cli --update`, `qwen-cli -u`)
